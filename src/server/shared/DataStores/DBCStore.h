...
//        T const* LookupEntry(uint32 id) const { return (id>=nCount)?NULL:indexTable[id]; }
        T* LookupEntryNoConst(uint32 id) const { return (id>=nCount)?NULL:indexTable.asT[id]; }
        void SetEntry(uint32 id, T* entry) { if (id>=nCount){ printf("ERROR"); return;} indexTable.asT[id]=entry; }
